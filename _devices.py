...
class QE65000(USB2000):
    ...
    def __init__(self):
        ...
        self._dev = usb.core.find(idVendor=0x2457, idProduct=0x1018)
        if self._dev is None:
            raise _OOError('No OceanOptics QE65000 spectrometer found!')
        else:
            print ('*NOTE*: Currently the first device matching the '
                   'Vendor/Product id is used')
        ...
        # This information comes from the OEM-Datasheet
        # "http://www.oceanoptics.com/technical"
        #       "/engineering/OEM%20Data%20Sheet%20--%2QE65000.pdf"
        self._EP1_out = 0x01
        self._EP1_in = 0x81
        self._EP2_in = 0x82
        self._EP6_in = 0x86
        ...
        #print(self._read_TEC_temperature())
        #print(self._get_TEC_settings())
        #print(self._set_TEC_temperature(-10))
        #print(self._read_temperatures())
    ...
    def acquire_spectrum(self):
        raw_intensity = self._request_spectrum()
        wavelength = sum( self._wl[i] * np.arange(0,1024)**i for i in range(4) )
        # fixed linearization, see documentation at:
        # --> http://www.oceanoptics.com/technical/OOINLCorrect%20Linearity%20Coeff%20Proc.pdf
        intensity =  raw_intensity / sum( (self._nl[i] * raw_intensity**i) for i in range(8) )
        return np.vstack([wavelength, intensity])
    ...
    def _read_TEC_temperature(self):
        ...
        self._dev.write(self._EP1_out, struct.pack('<B', 0x72))
        ret = self._dev.read(self._EP1_in, self._EP1_in_size)
    ...
    def _request_spectrum(self):
        ...
        time.sleep( max(self._it + 100000, 0) * 1e-6 )
        if self._usbcomm == 0x80: #HIGHSPEED
            ret = [self._dev.read(self._EP2_in, 512) for _ in range(5)]
            sync = self._dev.read(self._EP2_in, 1)
        else: # _usbcomm == 0x00  #FULLSPEED
            raise Exception('Fullspeed USB is not yet supported')

        ret = ret[0]+ret[1]+ret[2]+ret[3]+ret[4]
        spectrum = struct.unpack('<'+'H'*1280, ret)
        spectrum = np.array(spectrum, dtype=np.uint16)
        spectrum = spectrum[10:1034]
    ...
    def _get_saturation_calibration(self):
        #ret = self._query_information(0x11, raw=True)
        #return 65535.0/float(struct.unpack('<h',ret[6:8])[0])
        return 1 # QE65000 does not have that
    ...
    def _set_TEC_temperature(self, temperature):
        ...
        #print(self._get_TEC_settings()) # Read Settings
        self._dev.write(self._EP1_out, struct.pack('<BBB', 0x71, 0x00, 0x00)) # Disable TEC
        time.sleep(0.2) # wait 200ms
        #print(self._get_TEC_settings()) #Read Settings
        message = struct.pack('<B', 0x73)+(struct.pack('>h',(temperature*10)))
        self._dev.write(self._EP1_out, message) # Set Temp
        self._dev.write(self._EP1_out, struct.pack('<BBB', 0x70, 0x01, 0x00)) # Enable Fan
        self._dev.write(self._EP1_out, struct.pack('<BBB', 0x71, 0x01, 0x00)) # Enable TEC
        #print(self._get_TEC_settings()) #Read Settings
        time.sleep(2)
        #print(self._read_TEC_temperature()) # Read Temp
